...
class YzModelCode extends CCodeModel
{
	...
	public $baseClass='YzBaseModel';
    public $t9nCategory='Yz.t9n';
	public $buildRelations=true;
    public $createBehaviors=false;
    public $createCTimestampBehavior=true;
	...
	public function rules()
	{
		return array_merge(parent::rules(), array(
			...
			array('connectionId, tablePrefix, modelPath, baseClass, buildRelations, t9nCategory', 'sticky'),
			array('createBehaviors,createCTimestampBehavior', 'sticky'),
		));
	}
	...
	public function attributeLabels()
	{
		return array_merge(parent::attributeLabels(), array(
			...
			'createBehaviors'=>'Create Behaviors',
    ...
    public function generateBehaviors($table)
    {
        ...
        foreach($table->columns as $column){
            ...
            if( $column->name == 'update_date' && $this->createCTimestampBehavior )
                $timestampBehaviour['updateAttribute'] = $column->name;
            if( $column->name == 'create_date' && $this->createCTimestampBehavior )
                $timestampBehaviour['createAttribute'] = $column->name;
        }

        if( !empty($timestampBehaviour) )
            $behaviors[] = array(
                'name' => 'timestamp',
                'attributes' => $timestampBehaviour,
            );
