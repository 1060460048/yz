<?php
/**
 * YzMarkitupWidget adds {@link http://markitup.jaysalvat.com/ markitup} as a form field widget.
 *
...
class YzMarkitupWidget extends YzInputWidget
{
    /**
    ...
    public function init()
    {
        ...
        if($this->scriptUrl===null)
            $this->scriptUrl=Yii::app()->getAssetManager()->publish(dirname(__FILE__).'/MarkitupAssets',false,-1,Yz::get()->alwaysUpdateAssets);
    ...
    public function run()
    {
        if($this->hasModel()) {
            if($this->hasForm())
                echo $this->form->textAreaRow($this->model, $this->attribute, $this->htmlOptions);
            else
